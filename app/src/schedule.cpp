...
V2D file_to_V2D(const std::string & filename){
    ...
    for (unsigned i = 0; i < lines.size(); i++) {
        ...
        v2d.push_back(vector<string>());
        for (string el : elems) v2d[i].push_back(Trim(el));
    }
    return v2d;
}
...
V2D clean(const V2D & cv, const V2D & student){
    ...
    for (vector<string> v : student) {
        string s = v[0];
        cmap[s] = set<string>(v.begin() + 1, v.end()); // vector into set except the first element
    }

    for (vector<string> v : cv) {
        v2d.push_back(vector<string>());
        size_t l = v2d.size() - 1;
        ...
        for (unsigned i = 1; i < v.size(); i++) {
            string s = v[i];
            if (cmap.find(s) != cmap.end() && cmap[s].count(c)) {
                v2d[l].push_back(s);
            }
        }

        // remove if empty (only course, no students)
        if (v2d[l].size() == 1) v2d.erase(v2d.begin() + l);
    }
